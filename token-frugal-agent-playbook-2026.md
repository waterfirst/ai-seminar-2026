...
이번 특별편은 “멀티에이전트가 좋아 보인다”는 느낌이 아니라,  
**실제로 돌려 보고 토큰·속도·결과 차이를 비교한 기록**이다.

### 실험 A — 2026-07-09 장전 KOSPI 예측
...
- **멀티에이전트**
  - 구성: Gemini(시장 스캔) + Z AI(숫자 요약) + Codex(최종 합성)
  - 시가 예측: `7250`
  - 장전 종가 예측: `7230`
  - 사용량: **3487 tokens**
  - 시간: **약 19.6초**
...
- 시가 차이 `40p`, 장전 종가 차이 `50p`로 **결과 차이는 크지 않았다**
- 반면 비용은 멀티에이전트가 훨씬 컸다
- 따라서 **시가 전에는 Codex 단독이 더 경제적**이었다

### 실험 B — 세미나/서버 실무 작업

#### 1) 세미나 최신 자료 수집
- **Gemini → Codex**가 가장 자연스러웠다
- Gemini는 최신성 수집은 강하지만, 그대로 쓰면 출처 품질이 흔들릴 수 있다
- 그래서 Codex가 마지막에 선별해야 한다

#### 2) 세미나 문안/카드 초안
- **Z AI → Codex**가 가장 싸고 안정적이었다
- Z AI는 초안을 빠르게 만들고
- Codex는 톤, 설득력, 길이 조절에 강했다

#### 3) FastAPI/systemd/Nginx 같은 실무 코드
- 기본은 **Z AI 단독**
- 다만 운영 직전에는 **Gemini 리뷰**가 값어치가 있었다

### 실무 운영 규칙

1. **시가 전 예측은 Codex 단독 기본**
2. **12:30 이후 종가 판단은 Gemini(좁은 검색) + Codex**
3. **짧은 코드와 초안은 Z AI 먼저**
4. **배포 직전/운영 직전만 검토 모델 추가**

### 핵심 한줄

> **싼 초안은 Z AI, 최신 탐색은 Gemini, 비싼 판단은 Codex**  
> 이 조합이 현재 기준으로 가장 실용적이었다.
