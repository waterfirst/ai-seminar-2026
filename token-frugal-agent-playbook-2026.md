...
## 특별편. 단독 에이전트 vs 멀티에이전트 실사례

### 실험 A — 단순 수정은 Codex 단독

- 문구 한두 줄 수정
- 접기(`<details>`) 추가
- 밝은 톤으로 색상·배치 미세 조정
- 파일 위치 파악 → 수정 → 확인까지 한 번에 처리

#### 결론

- 작고 명확한 수정은 **Codex 단독**이 가장 빠르다
- 이 단계에서 멀티에이전트를 붙이면 설명 비용과 컨텍스트 비용이 커진다
- 따라서 **단순 수정은 단독 에이전트가 더 경제적**이다

### 실험 B — 자료 제작과 배포는 멀티에이전트

- **Gemini**: 최신 뉴스·외부 사례·링크 확인
- **Z AI**: 초안 문장, 짧은 요약, 대안 표현 생성
- **Codex**: HTML/MD 반영, 구조 정리, 중복 제거, 배포

즉 중요한 것은 “어떤 모델이 무조건 최고인가”보다,  
**누가 어느 단계에서 들어가야 하느냐**다.

### 실무 운영 규칙

1. **작은 수정은 Codex 단독**
2. **조사·정리·배포는 Gemini + Z AI + Codex**
3. **마지막 구조 판단과 승인은 사람**
4. 중요한 것은 “항상 멀티에이전트”가 아니라, **작업을 단계별로 다르게 설계하는 것**이다

### 핵심 한줄

> **단순 수정은 단독, 자료 제작은 멀티, 마지막 판단은 사람**  
> 이 구조가 현재 가장 실전적인 멀티에이전트 사용예다.

---

## 특별편 2. 현장의 목소리 — 라이선스는 개인 소유가 아니라 공공재다

AI를 위에서 강하게 밀어붙일수록 현장에서는 다른 온도가 감지된다. 사내 익명 게시판의 목소리는 불평이 아니라 **신호**다. 도구가 부족한 게 아니라 **배분과 목적이 어긋나 있다는 신호**다.

### 현장에서 나온 실제 목소리

- “오늘 전사 크레딧 소진됐대요 — 써보려니 이미 끝났다.” (밖을 잘 모르는 임원이 몰아 쓰면 실무자에겐 안 남는다)
- “문제를 먼저 놓아야지. Agent를 만들자를 먼저 정하고 문제를 찾으니 뭐든 Agent로 해보려다 낭비가 생긴다.”
- “자원이 뒷받침돼야 돌아가지. 개발만 하면 뭐하나, 쇼잉이지.”
- “필요한 데만 써야지.” (토큰당 과금 전환·예산 조기 소진 뉴스와 겹친 냉소)

> 현장의 목소리를 경청하지 않으면, 아무리 좋은 도구도 ‘윗선의 실적용 장식’으로 소비된다.

### 숫자로 본 배분의 현실

- **발표 30% vs 현실**: “2만 명 중 30%가 Codex를 쓴다”는 집계는 특정 부서 AI팀에 몰아준 결과. 실제 한 파트에서는 **17명 중 3명**만 쓴다.
- **1인 월 80만원**: Codex 크레딧 월 1만 개 ≈ 80만원 상당이 소수에게 간다. 개인 생산성으로만 소진되면 조직엔 남는 게 없다.
- **GitHub 1인 40만원/년**: 코드 보관비가 아니라 이미 쓴 토큰·시간을 자산으로 남기는 비용.

### 그래서 바꿔야 할 것 — 공공재로 재설계

- GitHub는 **50% 이상**, 개발 도구(Codex 등)는 **80% 이상** 되도록 넓게 배분한다.
- 크레딧을 받은 사람은 개인용을 넘어 **부서 공공재**로 쓰는 것을 전제로 한다.
- 공공재로 만든 웹서비스는 GitHub에 올려 **모두가 나눠 쓴다**.
- **스타(⭐) 문화 = 책임과 보상**: 재사용된 코드·앱에 Star로 발견 신호를 남기고, 기여자에게 보상(평가·인정)과 책임(유지·정확성)을 함께 준다. Star 수가 아니라 실제 재사용·개선 PR·절약 시간으로 평가한다.

핵심은 도구를 더 사느냐가 아니다. **소수에게 몰린 라이선스를 공공재로 돌리고, 현장의 목소리를 경청해 “진짜 필요한 문제”에 리소스를 붙이는 것**이다. 그때 비로소 크레딧이 개인의 소비가 아니라 조직의 자산이 된다.
